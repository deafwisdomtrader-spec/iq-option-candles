...
VERSAO_APP = "2026-09-01.8"

VERSAO_NOTAS = (
    "aquecimento no boot, conexao 25s, cadeado com prazo, "
    "reciclagem de pool, CORS liberado, /login-teste, "
    "aquecimento sem limite de tempo com novas tentativas"
)
...
# Guarda o que aconteceu no aquecimento, para /health mostrar.
_aquecimento = {
    "estado": "nao iniciado",
    "tentativas": 0,
    "erro": None,
...
def aquecer_conexao():
    """Conecta em segundo plano, SEM limite de tempo.

    Esta é a diferença que importa: aqui ninguém está esperando
    uma resposta HTTP, então a conexão pode demorar 60 ou 90
    segundos sem prejudicar nada.

    Nas requisições o limite continua existindo (25s), porque
    lá o usuário está esperando. Mas quando elas chegarem, a
    conexão já vai estar pronta e será apenas reutilizada.

    Tenta várias vezes, com intervalo crescente: se a corretora
    estiver instável, ele insiste sozinho em vez de deixar o
    serviço sem conexão até alguém recarregar a página.
    """
...
def health():
    ...
    return jsonify({
        ...
        # Como foi a conexão feita em segundo plano.
        # É aqui que aparece o motivo real de uma falha de
        # login, sem precisar de rota separada.
        "aquecimento":
            _aquecimento,
