...
# ============================================================
# TELEGRAM — ENVIO DE SINAIS
# ============================================================
# Configure no Render:
# TELEGRAM_BOT_TOKEN = token do BotFather
# TELEGRAM_CHAT_ID   = -1003944947776
#
# O token NÃO fica neste arquivo.
# ============================================================

TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "").strip()
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "-1003944947776").strip()
...
def telegram_enviar(texto):
    ...
    try:
        ...
        return bool(retorno.get("ok"))
    except Exception as erro:
        print("TELEGRAM ERRO:", str(erro)[:200])
        return False

def telegram_enviar_async(texto):
    threading.Thread(
        target=telegram_enviar,
        args=(texto,),
        daemon=True,
    ).start()
...
def telegram_formatar_sinal(par, analise):
    ...
    return (
        "🤖 <b>DW TRADING — NOVO SINAL</b>\n\n"
        f"💱 <b>{par}</b>\n"
        f"{emoji} <b>{sinal}</b>\n"
        f"⏰ Entrada: <b>{analise.get('entrada', '--:--')}</b>\n"
        "⏱️ M1\n\n"
        f"🎯 Confiança técnica: <b>{analise.get('confianca', 0)}</b>\n"
        f"📊 CALL {analise.get('pontos_call', 0)} × PUT {analise.get('pontos_put', 0)}\n"
        f"📌 Estratégia: {analise.get('estrategia', ESTRATEGIA)}\n\n"
        "⚠️ Análise educacional. Não garante resultado."
    )

def telegram_formatar_resultado(par, sinal, resultado):
    if resultado == "WIN":
        emoji = "✅"
    elif resultado == "LOSS":
        emoji = "❌"
...
def telegram_registrar_novo_sinal(par, analise):
    if not telegram_configurado():
        return
    ...
    with _telegram_lock:
        if chave in _telegram_pendentes:
            return
        _telegram_pendentes[chave] = {
            "par": str(par).upper(),
            "sinal": sinal,
            "entrada_em": int(entrada_em),
        }

    telegram_enviar_async(telegram_formatar_sinal(par, analise))

def telegram_processar_resultado(par, inicio_candle, resultado):
    if resultado not in ("WIN", "LOSS", "EMPATE"):
        return

    chave = f"{str(par).upper()}|{int(inicio_candle)}"

    with _telegram_lock:
        pendente = _telegram_pendentes.pop(chave, None)

    if not pendente:
        return

    telegram_enviar_async(
        telegram_formatar_resultado(
            pendente["par"],
            pendente["sinal"],
            resultado,
        )
...
def resultado_sinal(par):
    ...
    if m1 in ("WIN", "LOSS", "EMPATE"):
        telegram_processar_resultado(par, inicio_candle, m1)
